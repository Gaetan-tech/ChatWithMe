  ...
  const loadUsers = async () => {
    try {
      ...
      if (user?.flagStatus === 'green') {
        // Si j'ai le flag vert, je vois les utilisateurs avec flag rouge
        const response = await usersAPI.getRedFlagUsers();
        setRedFlagUsers(response.data.users || []);
        setGreenFlagUsers([]);
      } else if (user?.flagStatus === 'red') {
        // Si j'ai le flag rouge, je vois les utilisateurs avec flag vert
        const response = await usersAPI.getGreenFlagUsers();
        setGreenFlagUsers(response.data.users || []);
        setRedFlagUsers([]);
      } else {
        setRedFlagUsers([]);
        setGreenFlagUsers([]);
